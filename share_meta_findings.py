...
def build_share_meta_findings(signals: dict, lang: str = "en") -> list[dict[str, Any]]:
    ...
    if share.get("error"):
        findings.append({
            ...
            "description_ro": "Metadatele Open Graph/previzualizare nu au putut fi extrase în mod fiabil pentru această pagină.",
    ...
    if missing_tw:
        findings.append({
            ...
            "title_ro": "Lipsesc metadatele de previzualizare",
            "description_en": "twitter:card is not present. Many platforms use Open Graph, but Twitter/X uses Twitter card metadata.",
            "description_ro": "Lipsește un tag de previzualizare pentru partajare. Multe platforme folosesc Open Graph, dar unele folosesc tag-uri suplimentare.",
            "recommendation_en": "Add twitter:card (and twitter:title/description/image) for consistent previews on Twitter/X.",
            "recommendation_ro": "Adăugați metadate de previzualizare complete pentru consistență la partajare.",
        ...
        if card and card not in ALLOWED_TW_CARDS:
            findings.append({
                ...
                "title_ro": "Valoare neobișnuită pentru tag-ul de previzualizare",
                "description_en": "twitter:card has a value that is not commonly used. This can lead to unexpected preview behavior.",
                "description_ro": "Tag-ul de previzualizare are o valoare rar folosită. Acest lucru poate duce la previzualizări neașteptate.",
    ...
    if dup_tw:
        findings.append({
            ...
            "title_ro": "Tag-uri de previzualizare duplicate detectate",
            "description_en": "Multiple values were found for one or more Twitter card tags. Twitter/X may choose the wrong one.",
            "description_ro": "Au fost găsite mai multe valori pentru unul sau mai multe tag-uri de previzualizare. Platformele pot alege valoarea greșită.",
            "recommendation_en": "Keep a single value per Twitter card tag (especially twitter:card, title, description, image).",
            "recommendation_ro": "Păstrați o singură valoare per tag de previzualizare (mai ales titlu, descriere, imagine).",
    ...
    if not missing_og and not missing_tw:
        findings.append({
            ...
            "description_ro": "Metadatele Open Graph și de previzualizare sunt prezente pe homepage.",
            "recommendation_en": "Periodically check that previews render correctly on Facebook/LinkedIn/WhatsApp and Twitter/X.",
            "recommendation_ro": "Verificați periodic că previzualizările se redau corect pe Facebook/Instagram/LinkedIn/WhatsApp.",
